...
def compose_poster(scene_path, dst, title: str, birds, *, size=(3840, 2160)) -> Path:
    ...
    scene = scene.resize((nw, nh), Image.LANCZOS)
    ...
    while _text_width(draw, title, tfont) > width * 0.8 and tsize > 16:
        tsize -= 3
        tfont = _font(_SERIF_BOLD, tsize)
    tw = _text_width(draw, title, tfont)
    draw.text(((width - tw) / 2, (top_h - tsize) / 2), title, font=tfont, fill=_INK)

    # Species legend in the bottom margin: up to 5 per row, common + italic Latin.
    n = len(birds)
    ...
    for ri, row in enumerate(rows):
        ...
        for ci, bird in enumerate(row):
            ...
            draw.text((cx - cw / 2, y), common, font=cfont, fill=_INK)
            if bird.common:
                sw = _text_width(draw, bird.scientific, ifont)
                draw.text((cx - sw / 2, y + common_sz * 1.12), bird.scientific, font=ifont, fill=_INK)
